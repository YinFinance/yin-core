...
abstract contract LockLiquidity {
    ...
    modifier afterLockUnsubscribe(uint256 yangId, uint256 chiId) {
        require(_isUnLocked[chiId] || (!_isUnLocked[chiId] && block.timestamp > _locks[yangId][chiId]), 'locks');
        _;
    }
    ...
    function _updateAccountLockDurations(uint256 yangId, uint256 chiId, uint256 currentTime) internal {
        if (!_isUnLocked[chiId]) {
            uint256 durationTime = currentTime.add(_lockInSeconds);
            _locks[yangId][chiId] = durationTime > _locks[yangId][chiId] ? durationTime : _locks[yangId][chiId];
            emit LockAccount(yangId, chiId, _locks[yangId][chiId]);
        }
    }

    function durations(uint256 yangId, uint256 chiId) external view returns (uint256) {
