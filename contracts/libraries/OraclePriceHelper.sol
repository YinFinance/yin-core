...
library OraclePriceHelper {
    ...
    function isReachMaxUSDLimit(
        ...
        if (maxUSDLimit == 0) {
            ...
            (uint256 amount0, uint256 amount1) = ICHIVault(chiVault)
                .getTotalAmounts();
            (int256 price0, int256 price1) = IOracleProvider(oracle)
                .getPairUSDPrice(token0, token1);
            uint256 totalBalance = amount0.mul(uint256(price0)).add(
                amount1.mul(uint256(price1))
            );
